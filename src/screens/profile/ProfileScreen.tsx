      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.avatarSection}>
          <View
            style={[
              styles.avatar,
              { backgroundColor: roleColor[appUser?.role ?? 'user'] + '33' },
            ]}
          >
            <ThemedText
              style={[
                styles.initials,
                { color: roleColor[appUser?.role ?? 'user'] },
              ]}
            >
              {initials}
            </ThemedText>
          ...
          {appUser && (
            <View style={styles.badges}>
              <StatusBadge
                label={ROLE_LABELS[appUser.role]}
                color={roleColor[appUser.role]}
              />
              <StatusBadge
                label={STATUS_LABELS[appUser.status]}
                color={userStatusColor[appUser.status]}
              />
            </View>
          )}
        </View>
        ...
        <View style={styles.card}>
          <InfoRow icon="call-outline" label="Teléfono" value={appUser?.phone ?? '—'} />
...
const styles = StyleSheet.create({
  ...
  name: { fontSize: 22, fontWeight: '700', color: AppColors.textPrimary },
  email: { fontSize: 14, color: AppColors.textMuted },
  ...
  logoutText: { color: AppColors.error, fontWeight: '700', fontSize: 15 },
});
