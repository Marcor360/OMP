...
import { Assignment, AssignmentStatus, ASSIGNMENT_PRIORITY_LABELS, ASSIGNMENT_STATUS_LABELS } from '@/src/types/assignment';
...
export function AssignmentDetailScreen() {
  ...
  if (error || !assignment) return <ErrorState message={error ?? 'Asignacion no encontrada.'} />;
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.titleSection}>
          <View style={styles.badgeRow}>
            <StatusBadge label={ASSIGNMENT_STATUS_LABELS[assignment.status]} color={assignmentStatusColor[assignment.status]} />
            <StatusBadge label={ASSIGNMENT_PRIORITY_LABELS[assignment.priority]} color={priorityColor[assignment.priority]} />
          </View>
          <ThemedText style={styles.title}>{assignment.title}</ThemedText>
          {assignment.description ? <ThemedText style={styles.description}>{assignment.description}</ThemedText> : null}
        </View>
        ...
        <View style={styles.card}>
          ...
          <InfoRow
            ...
            valueStyle={overdue ? { color: colors.error } : undefined}
          />
        ...
        {nextStatus ? (
          <RoleGuard allowedRoles={['admin', 'supervisor']}>
            <TouchableOpacity style={styles.advanceBtn} onPress={handleAdvanceStatus} disabled={updating} activeOpacity={0.8}>
...
const createStyles = (colors: AppColorSet) =>
  StyleSheet.create({
    ...
    advanceBtnText: { color: colors.success, fontWeight: '600' },
    deleteBtn: {
      flexDirection: 'row',
      alignItems: 'center',
      gap: 10,
